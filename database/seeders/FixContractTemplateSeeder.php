...
class FixContractTemplateSeeder extends Seeder
{
    public function run(): void
    {
        // Contenido HTML Limpio y Profesional
        $htmlContent = <<<'HTML'
...
HTML;
        ...
        // Actualizar o Crear la plantilla para TODOS los tenants (En producción se haría solo para el actual, pero aquí asumimos contexto local de User)
        // Buscamos documentos CONTRATO_SERVICIOS con tenant_id NO nulo (es decir, los de los despachos)
        // Y les actualizamos el contenido.
        
        $documentos = LegalDocument::where('tipo', 'CONTRATO_SERVICIOS')
            ->whereNotNull('tenant_id')
            ->get();

        if ($documentos->count() > 0) {
            foreach ($documentos as $doc) {
                $doc->update(['texto' => $htmlContent]);
            }
        } else {
            // Si no hay, creamos uno para el tenant del usuario (si hay usuario autenticado) o genérico
             // Pero como es un seeder, vamos a crear uno para el primer tenant que encontremos o dejarlo global si no hay tenants
             // Asumiremos que el usuario ya tiene uno creado y solo quiero arreglarlo.
             
             // Intento de encontrar el del usuario actual si estuviera logueado (en seeder no aplica auth()).
             // Vamos a buscar el último creado.
             $lastDoc = LegalDocument::where('tipo', 'CONTRATO_SERVICIOS')->latest()->first();
             if ($lastDoc) {
                 $lastDoc->update(['texto' => $htmlContent]);
             }
        }
        
        // También actualizar la plantilla global por si acaso
        /*
        LegalDocument::updateOrCreate(
            ['tipo' => 'CONTRATO_SERVICIOS', 'tenant_id' => null],
            [
                'nombre' => 'Plantilla Base de Contrato de Servicios',
                'texto' => $htmlContent,
                'version' => '1.0',
                'activo' => true,
