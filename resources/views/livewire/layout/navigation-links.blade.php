@php
    $user = auth()->user();
    $plan = $user->tenant ? $user->tenant->plan : null;
    
    // Es usuario exclusivo de directorio? (No tiene plan de despacho)
    $isPureDirectoryUser = $plan && str_contains($plan, 'directory');
    
    // Tiene acceso a las funciones de Despacho (Diogenes)?
    // Si es super_admin, siempre. Si no es pure directory, también.
    $hasOfficeAccess = $user->hasRole('super_admin') || !$isPureDirectoryUser;
    
    // Todos los abogados/admin pueden ver su sección de directorio
    $canSeeDirectory = $user->hasRole(['super_admin', 'admin', 'abogado']);
@endphp
...
<nav class="space-y-1">
    ...
    <div class="pb-4">
        ...
        @if($canSeeDirectory)
            <x-sidebar-link :href="route('directory.dashboard')" :active="request()->routeIs('directory.dashboard')" icon="globe">
                {{ __('Mi Directorio') }}
            </x-sidebar-link>
        @endif

        @if($hasOfficeAccess)
            <x-sidebar-link :href="route('projects.index')" :active="request()->routeIs('projects.*')" icon="clipboard">
                {{ __('Proyectos') }}
            </x-sidebar-link>
        ...
        <x-sidebar-link :href="route('profile.directory')" :active="request()->routeIs('profile.directory')" icon="globe">
            {{ __('Perfil Público') }}
        </x-sidebar-link>
    ...
    <div class="pt-4 border-t border-gray-300">
        ...
        <x-sidebar-link :href="route('admin.directory.index')" :active="request()->routeIs('admin.directory.*')" icon="users">
            {{ __('Directorio Público') }}
        </x-sidebar-link>
    </div>
    @endif
