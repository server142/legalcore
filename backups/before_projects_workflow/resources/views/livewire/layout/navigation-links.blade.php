@php
    ...
    $isDirectoryUser = $plan && str_contains($plan, 'directory');
@endphp
...
<nav class="space-y-1">
    ...
    <div class="pb-4">
        ...
        @if($isDirectoryUser)
            <x-sidebar-link :href="route('directory.dashboard')" :active="request()->routeIs('directory.dashboard')" icon="dashboard">
                {{ __('Mi Directorio') }}
            </x-sidebar-link>
        @endif

        @if(auth()->user()->hasRole(['super_admin', 'admin', 'abogado']) && !$isDirectoryUser)
