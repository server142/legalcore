<div class="min-h-screen bg-gray-50 py-12">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-16 bg-white rounded-2xl shadow-xl overflow-hidden mb-12">
                <div class="grid grid-cols-1 md:grid-cols-3 divide-y divide-gray-200 md:divide-y-0 md:divide-x divide-gray-200">
                    @foreach($availablePlans as $p)
                        <div class="p-8 flex flex-col justify-between hover:bg-gray-50 transition-colors relative group">
                            ...
                            <a href="{{ route('billing.subscribe', $p->slug) }}?context={{ $context }}" class="mt-8 block w-full rounded-xl py-4 px-6 text-center text-sm font-bold transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 {{ $btnClass }}">
                                Seleccionar {{ $p->name }}
                            </a>
                        </div>
                    @endforeach
             ...
             <div class="text-center mt-8">
                <a href="{{ $context === 'directory' ? route('directory.dashboard') : route('dashboard') }}" class="text-sm text-gray-500 hover:text-gray-900 font-medium">← Volver al Dashboard</a>
            </div>
        </div>
    ...
    @else
        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-8">
                <a href="{{ route('billing.subscribe', 'trial') }}?context={{ $context }}" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-indigo-600 transition-colors">
