    ...
    <div class="py-12 bg-gray-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-8">
                <div class="lg:w-1/4 w-full mb-8 lg:mb-0">
                    <div class="sticky top-32">
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-200/60 p-6 flex flex-col max-h-[calc(100vh-140px)]">
                            <h3 class="text-[10px] font-extrabold text-indigo-600 uppercase tracking-[0.2em] mb-6 shrink-0">Contenido del Manual</h3>
                            <nav class="space-y-1.5 overflow-y-auto pr-2 custom-scrollbar flex-1">
                                @foreach($pages as $page)
                                    <a href="#{{ $page->slug }}" 
                                       onclick="event.preventDefault(); document.getElementById('{{ $page->slug }}').scrollIntoView({ behavior: 'smooth' });"
                                       class="group flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-gray-500 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-xl transition-all duration-200 border border-transparent hover:border-indigo-100/50">
                                        <div class="w-1.5 h-1.5 rounded-full bg-gray-300 group-hover:bg-indigo-500 shadow-sm shrink-0 transition-colors"></div>
                ...
                <div class="lg:w-3/4 space-y-8">
                    ...
                    @forelse($pages as $page)
                        <article id="{{ $page->slug }}" class="scroll-mt-32 bg-white rounded-3xl shadow-sm border border-gray-200/60 overflow-hidden hover:shadow-md transition-shadow duration-300">
                            <!-- Header del Post -->
        ...
        <script>
            // Simple script para deep linking
            document.addEventListener('DOMContentLoaded', () => {
                 const urlParams = new URLSearchParams(window.location.search);
                 const pageSlug = urlParams.get('page');
                 if (pageSlug) {
                     setTimeout(() => {
                         const element = document.getElementById(pageSlug);
                         if (element) {
